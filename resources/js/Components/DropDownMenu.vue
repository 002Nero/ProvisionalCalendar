...
const visibleTeachings = computed(() => {
    ...
    return teachings.value.filter((teaching) => {
        const matchesSearch = teaching.name
            .toLowerCase()
            .includes(searchValue.value.toLowerCase());
        const matchesPeriod =
            selectedPeriodId.value === 0 ||
            teaching.period?.id === selectedPeriodId.value;
...
const handleNextPeriod = () => {
    selectedPeriodId.value =
        selectedPeriodId.value === periods.length - 1
            ? 0
    ...
    <div class="dropdown-menu relative">
        ...
        >
            {{
                selectedTeaching
                    ? teachings.find(
                          (t) => t.id!.toString() === selectedTeaching
                      )?.name
                    : "Sélectionner un enseignement"
            }}
        </button>
        ...
        >
            ...
            <div class="max-h-60 overflow-y-auto">
                <div
                    ...
                    @click="
                        handleSelect({
                            target: { value: teaching.id!.toString() },
                        } as any)
                    "
