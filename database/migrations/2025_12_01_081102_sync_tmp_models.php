...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            ...
            try {
                ...
                if ($driver === 'mysql') {
                    ...
                    // SQLite: désactiver les FK et lister les tables via sqlite_master
                    DB::statement('PRAGMA foreign_keys = OFF;');
                    $rows = DB::select("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%';");
                    $existing = array_map(function ($r) {
                        return is_object($r) ? $r->name : (array_values((array)$r)[0] ?? null);
                    }, $rows);
                } else {
                    // Fallback: tenter la méthode MySQL / SHOW TABLES
                    DB::statement('SET FOREIGN_KEY_CHECKS=0;');
                    $rows = DB::select('SHOW TABLES');
                    $existing = array_map(function ($r) {
                        $a = (array) $r;
                ...
                // On remettra FOREIGN_KEY_CHECKS / PRAGMA à 1 plus bas après la recréation
            } catch (\Exception $e) {
                // Si la méthode de listing/desactivation n'est pas disponible, on ignore
            }
        ...
        try {
            ...
            // Ignore if we can't re-enable foreign keys
        }
    }
