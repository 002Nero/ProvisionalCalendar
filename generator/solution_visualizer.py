...
class SolutionVisualizer:
    ...
    def _build_planning_from_solution(self):
        ...
        for c in self.data['cours']:
            ...
            # Récupérer TOUS les profs assignés au cours (au lieu d'un seul)
            # Les clés de z_prof sont des tuples (cid, p_idx), donc on extrait p_idx (index [1])
            p_indices = [p_idx for (course_id, p_idx), v in self._vars['z_prof'].items() if
                         course_id == cid and self.solver.Value(v)]
            profs_str = ", ".join([self.data['profs'][p_idx] for p_idx in p_indices]) if p_indices else "UNKNOWN"
            
            if s_idx is not None and r_idx is not None and p_indices:
                self.actual_starts[cid] = s_idx
                salle_str = list(self.data['salles'].keys())[r_idx]
                for offset in range(self.data['duree_cours'][cid]):
                    planning[s_idx + offset].append((cid, salle_str, profs_str))
        return planning
    ...
    def _print_schedule_to_console(self):
        ...
        for d_idx in range(self.data['jours']):
            ...
            for t_in_day in range(self.data['creneaux_par_jour']):
                ...
                if entries:
                    for (cid, room_str, teachers_str) in entries:
                        if self.actual_starts.get(cid) == global_t:
                            print(f"  {time_str} : {cid} (Room: {room_str}, Teachers: {teachers_str}) Début")
                            ...
                            print(f"  {time_str} : {cid} (Room: {room_str}, Teachers: {teachers_str})")
